...
function addGroupField(container,name,groups,value) {
	...
	if (value != null) {
		jQuery('#'+random+' input').val(value);
	}
	if (groups != null) {
		jQuery('#'+random+' select option[value='+groups+']').attr('selected','selected');
	}
}
<?php endif; ?>
</script>
    ...
    <tr>
        ...
        <td>
            <select id='bootstrap_loading' name='params_bootstrap_loading'>
                ...
                <option value='0' <?php if (@$this->conf->bootstrap_loading == 0) { echo "selected"; } ?>><?php echo JText::_('ADSMANAGER_BOOTSTRAP_MINIMAL'); ?></option>
