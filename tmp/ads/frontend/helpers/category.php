...
class JHTMLAdsmanagerCategory {
	...
	static function displayColorCategories($name="category",$listcats,$catid,$options=array(),$attr=array()) {
		$defaultoptions = array("root_allowed"=>true,"display_price" => false,"color" => array('#dcdcc3'),"allow_empty"=>false);
		foreach($options as $key => $value) {
			$defaultoptions[$key] = $value;
		}
		$options = $defaultoptions;
		...
		?>
		<select id="<?php echo htmlspecialchars($name)?>" name="<?php echo htmlspecialchars($name)?>" <?php echo $attrhtml?>>
	...
	static function displayNormalCategories($name="category",$listcats,$catid,$options=array(),$attr=array()) {
		$defaultoptions = array("root_allowed"=>true,"display_price" => false,"separator" => " > ","allow_empty"=>false);
		foreach($options as $key => $value) {
			$defaultoptions[$key] = $value;
		}
		$options = $defaultoptions;
