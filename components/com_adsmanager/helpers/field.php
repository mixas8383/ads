	...
	function showFieldForm($field,$content,$default)
	{
		...
		switch($field->type)
		{
			...
			case 'price':
                ...
                if ($field->required == 1) {
					$input = "<input class='adsmanager_required form-control' required id='$name' type='text' test='number'  name='$name' size='$field->size' maxlength='$field->maxlength' $read_only value=\"$value\" placeholder=\"$placeholder\" />\n"; 
                } else {
					$input = "<input class='adsmanager form-control' id='$name' type='text' name='$name' test='number'  size='$field->size' maxlength='$field->maxlength' $read_only value=\"$value\" placeholder=\"$placeholder\" />\n";
                }
